...
function UserDetailsProvider({ children }) {
  ...
  const login = async (data, from, setLoading) => { //actions,
    try {
      setLoading(true)
  ...
  const signup = async (data, from, setLoading) => { //, actions, setSuccessModal
    try {
      setLoading(true)
  ...
  const handleChangePassowrd = async (data, actions, next) => {
    try {
      const res = await api.post(appUrls.PASSWORDACTIVATE_URL, data);
      if (res?.data?.code === "200") {
        actions.setSubmitting(false);
        next(true);
      }
